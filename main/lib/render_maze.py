"""
This script captures an image from a RealSense camera. 
If the device does have an RGB sensor, it configures the stream
for the sensor and starts streaming. It then waits for frames and retrieves the color frame. If no color frame
is received, it prints an error message and returns. The captured images are then converted to numpy arrays.
"""
...
def capture_image():
   """this function captures an image from a RealSense camera.
   """
   ...
   try:
      while True:
         ...
         cv2.namedWindow('Use the space bar to take a picture of the maze!', cv2.WINDOW_AUTOSIZE)
         
         cv2.imshow('Use the space bar to take a picture of the maze!', color_image)
...
def prepare_image():
   """this function prepares the image for processing.
   """
...
def convert_to_binary():
   """This function converts the image to binary.

   Returns:
      binary_image_1d: the binary image as a 1D numpy array
   """
